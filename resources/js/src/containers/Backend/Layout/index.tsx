...
class Layout extends Component<Props> {
    ...
    selectItem = (item: string) => this.setState({ selectedItem: item });

    render() {
        if (!this.props.auth || !this.props.content) return;
        ...
        footerContent = (
            <>
                ...
                <span className={`text-700 text-${dark ? "black" : "border"}`}>
                    {app_name}
                </span>
                . {all_rights} <img src={company_logo} style={{ height: 47 }} />
                .
            </>
        );
        ...
        return (
            <div className="Backend Layout text-left text-secondary">
                ...
                <main
                    className={`bg-${
                        dark ? "black" : "white"
                    } position-relative min-vh-100 pb-4 pb-sm-5`}
                >
                    <div className="toolbar-height row m-0 align-items-stretch border-bottom border-light sticky-top bg-white">
                        <div className="col-8 px-3 px-md-4 px-xxl-5 d-flex align-items-center">
                            <div className="text-black text-30">
                                <i
                                    onClick={this.toggle}
                                    className="fad fa-th-large text-border cursor-pointer"
                                />
                            </div>
                            ...
                            <div className="ml-auto mr-3 mr-md-4 mr-xxl-5 pr-1 pr-md-2 pr-xxl-3">
                                <div className="position-relative">
                                    <Input
                                        type="search"
                                        placeholder="Search"
                                        className="rounded-pill border-0 text-12 text-md-15 text-xxl-18 text-secondary bg-soft py-1 py-md-2 px-2 px-md-3 px-xxl-4"
                                    />

                                    <button
                                        className="fas fa-search text-border text-15 text-md-19 text-xxl-23 border-0 bg-transparent p-0 mr-2 mr-md-3 mr-xxl-4 position-absolute"
                                        style={{
                                            top: "50%",
                                            right: 0,
                    ...
                    <footer
                        className={`position-absolute d-none d-sm-block w-100 py-3 px-4 bg-${
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
    logout: () => dispatch(authLogout() as unknown as UnknownAction),
    set: (id: string) => dispatch(setLanguage(id) as unknown as UnknownAction),
});
