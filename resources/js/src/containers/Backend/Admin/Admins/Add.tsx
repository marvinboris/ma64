...
import {
    ...
    patchAdmins,
    resetAdmins,
} from "../../../../store/actions/backend/admins";
...
class Add extends Component<BackendAdminPage> {
        ...
        if (input.files && input.files[0]) {
            ...
            reader.onload = function (e) {
                ...
                if (embed) {
                    ...
                    const fileSelected = embed.querySelector(".file-selected");
                    if (fileSelected) fileSelected.innerHTML = file.name;
                }
            };
    ...
    componentDidUpdate(prevProps: BackendAdminPage) {
        if (
            !prevProps.backend?.admins?.message &&
            this.props.backend?.admins?.message &&
            this.props.backend.admins.message.type === "success" &&
            !this.props.edit
        ) {
        ...
        if (
            !prevProps.backend?.admins?.admin &&
            this.props.backend?.admins?.admin
        ) {
            const {
                backend: {
                    admins: { admin },
        ...
        if (
            ...
            if (!this.props.backend.admins.admin?.photo) return;
    ...
    render() {
        if (!this.props.content || !this.props.backend?.admins) return null;
        ...
        let {
            content: {
                cms: {
                    pages: {
                        backend: {
                            pages: {
                                admins: { title, add, edit, index, form },
                            },
                        },
                    },
                },
            ...
            backend: {
                admins: { loading, error, message, admin },
            },
        } = this.props;
        let { name, phone, photo, email, password, password_confirmation } =
            this.state;
        let content;
        let errors = null;
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
    get: (id?: string | number) =>
        dispatch(getAdmin(id) as unknown as UnknownAction),
    post: (data: object) =>
        dispatch(postAdmins(data) as unknown as UnknownAction),
    patch: (id: string, data: object) =>
        dispatch(patchAdmins(id, data) as unknown as UnknownAction),
