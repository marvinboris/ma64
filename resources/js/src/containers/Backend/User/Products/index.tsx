...
class Index extends Component<BackendPage> {
    ...
    render() {
        ...
        let {
            ...
            backend: {
                products: { loading, error, message, products, total },
            },
            auth: {
                data: {
                    role: { features },
                ...
                photo: product.photo && (
                    <div className="d-flex">
                        ...
                        <span className="ml-auto">
                            <View
                                title={`${form.product_photo}: ${product.name[lang]}`}
                                content={
                                    <img
                                        src={product.photo}
                                        className="w-100"
                ...
                action: (
                    <div className="text-center">
                        ...
                        {JSON.parse(feature?.permissions || "[]").includes(
                            ...
                            <Link
                                to={`/user/products/${product.id}/edit`}
                                className="mx-1"
                            >
                                <FontAwesomeIcon
                                    icon={faEdit}
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
    get: (page?: string | number, show?: string | number, search?: string) =>
        dispatch(getProducts(page, show, search) as unknown as UnknownAction),
    delete: (id: string) =>
        dispatch(deleteProducts(id) as unknown as UnknownAction),
