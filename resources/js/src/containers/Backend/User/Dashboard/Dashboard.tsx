...
class Dashboard extends Component<BackendAdminPage> {
    componentDidMount() {
        this.props.get?.();
    }
    ...
    render() {
        ...
        if (loading)
            ...
            if (blocksData) {
                ...
                const restaurantsData = totalRestaurants?.map((restaurant) =>
                    updateObject(restaurant, {
                        action: (
                            <div className="text-center">
                                <Link
                                    className="text-blue mr-2"
                                    to={`/user/restaurants/${restaurant.id}`}
                                >
                                    <FontAwesomeIcon icon={faEye} fixedWidth />
                                </Link>
                                <Link
                                    className="text-green mr-2"
                                    to={`/user/restaurants/${restaurant.id}/edit`}
                                >
                ...
                const plansData = totalPlans?.map((restaurant) =>
                    updateObject(restaurant, {
                        action: (
                            <div className="text-center">
                                <Link
                                    className="text-blue mr-2"
                                    to={`/user/plans/${restaurant.id}`}
                                >
                                    <FontAwesomeIcon icon={faEye} fixedWidth />
                                </Link>
                                <Link
                                    className="text-green mr-2"
                                    to={`/user/plans/${restaurant.id}/edit`}
                                >
                                    <FontAwesomeIcon icon={faEdit} fixedWidth />
                                </Link>
                            </div>
                ...
                const rechargesData = totalRecharges?.map((restaurant) =>
                    updateObject(restaurant, {
                        action: (
                            <div className="text-center">
                                <Link
                                    className="text-blue mr-2"
                                    to={`/user/recharges/${restaurant.id}`}
                                >
                                    <FontAwesomeIcon icon={faEye} fixedWidth />
                                </Link>
                                <Link
                                    className="text-green mr-2"
                                    to={`/user/recharges/${restaurant.id}/edit`}
                                >
                                    <FontAwesomeIcon icon={faEdit} fixedWidth />
                                </Link>
                            </div>
                ...
                content = (
                    <>
                        ...
                        <Row className="mt-5">
                            ...
                            >
                                <Link to="/user/plans" className="text-reset">
                                    {"View full plan list | >"}
                                </Link>
                            </Table>
