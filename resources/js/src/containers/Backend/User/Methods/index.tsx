...
class Index extends Component<BackendPage> {
    ...
    render() {
        ...
        let {
            ...
            backend: {
                methods: { loading, error, message, methods, total },
            },
            auth: {
                data: {
                    role: { features },
                ...
                action: (
                    <div className="text-center">
                        ...
                        {JSON.parse(feature?.permissions || "[]").includes(
                            ...
                            <Link
                                to={`/user/methods/${method.id}/edit`}
                                className="mx-1"
                            >
                                <FontAwesomeIcon
                                    icon={faEdit}
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
    get: (page?: string | number, show?: string | number, search?: string) =>
        dispatch(getMethods(page, show, search) as unknown as UnknownAction),
    delete: (id: string) =>
        dispatch(deleteMethods(id) as unknown as UnknownAction),
