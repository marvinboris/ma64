...
class Index extends Component<BackendPage> {
    ...
    render() {
        ...
        let {
            content: {
                cms: {
                    pages: {
                        ...
                        backend: {
                            pages: {
                                languages: { title, add, index, form },
                            },
                        },
                    },
                },
            ...
            backend: {
                languages: { loading, error, message, languages, total },
            },
            auth: {
                data: {
                    role: { features },
        ...
        const data = languages.map((language) => {
            return updateObject(language, {
                ...
                flag: (
                    <div>
                        <span
                            className={`flag-icon flag-icon-${language.flag.toLowerCase()} mr-1`}
                        />
                        {language.flag}
                    </div>
                ),
                action: (
                    <div className="text-center">
                        ...
                        {JSON.parse(feature?.permissions || "[]").includes(
                            ...
                            <Link
                                to={`/user/languages/${language.id}/edit`}
                                className="mx-1"
                            >
                                <FontAwesomeIcon
                                    icon={faEdit}
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
    get: (page?: string | number, show?: string | number, search?: string) =>
        dispatch(getLanguages(page, show, search) as unknown as UnknownAction),
    delete: (id: string) =>
        dispatch(deleteLanguages(id) as unknown as UnknownAction),
