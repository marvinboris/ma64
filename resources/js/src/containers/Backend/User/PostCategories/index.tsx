            ...
            content: {
                cms: {
                    pages: {
                        ...
                        backend: {
                            pages: {
                                post_categories: { title, add, index, form },
                            },
                        },
                    },
                },
            ...
            return updateObject(post_category, {
                ...
                action: (
                    <div className="text-center">
                        <Link
                            to={`/user/post-categories/${post_category.id}`}
                            className="mx-1"
                        >
                            <FontAwesomeIcon
                                icon={faEye}
                        ...
                        {JSON.parse(feature?.permissions || "[]").includes(
                            ...
                            <Link
                                to={`/user/post-categories/${post_category.id}/edit`}
                                className="mx-1"
                            >
                                <FontAwesomeIcon
                                    icon={faEdit}
                        ...
                        {JSON.parse(feature?.permissions || "[]").includes(
                            ...
                            <span className="mx-1">
                                <Delete
                                    deleteAction={() =>
                                        this.props.delete(post_category.id)
                                    }
                                >
                                    <FontAwesomeIcon
                                        icon={faTrash}
            ...
            <>
                <Row>
                    <List
                        array={data}
                        loading={loading}
                        data={JSON.stringify(post_categories)}
                        get={this.props.get}
                        total={total}
                        bordered
                        add={add}
                        link="/user/post-categories/add"
                        icon={icon}
                        title={index}
                        className="shadow-sm"
                        fields={[
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
    get: (page?: string | number, show?: string | number, search?: string) =>
        dispatch(
            getPostCategories(page, show, search) as unknown as UnknownAction
        ),
    delete: (id: string) =>
        dispatch(deletePostCategories(id) as unknown as UnknownAction),
