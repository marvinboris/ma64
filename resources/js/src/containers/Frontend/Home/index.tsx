...
class Home extends Component<State & { get: () => void }> {
    componentDidMount() {
        this.props.get();
    }

    render() {
        if (!this.props.content || !this.props.frontend?.home) return;
        const {
            content: {
                cms: {
                    pages: {
            ...
            frontend: {
                home: { loading, error, message, products = [], posts = [] },
            },
        } = this.props;
        let content;
        ...
        const articleCards = products.map((product) => (
            <ArticleCard
                key={Math.random() + JSON.stringify(product)}
                {...(updateObject(product, {
                    name: product.name[lang],
                    description: product.description[lang],
                }) as Omit<Product, "name" | "description"> & {
                    name: string;
                    description: string;
                })}
            />
        ...
        const newsCards = posts.map((post) => (
            <Post
                ...
                }) as Omit<PostType, "title" | "body"> & {
                    title: string;
                    body: string;
                })}
            />
        ));
        const characteristicsItems = qualification_assistance.points.map(
            (point, index) => (
                <Characteristic key={point} number={index + 1}>
                    {point}
                </Characteristic>
            )
        );
                            ...
                            <Container className="h-100">
                                ...
                                >
                                    <Col lg={6} className="pl-4">
                                        ...
                                        >
                                            <CenterButton pill color="dark">
                                                {
                                                    squares.education_kits
                                                        .ask_price
                                                }
                                            </CenterButton>
                                        </a>
                                ...
                                >
                                    <Col lg={6} className="pr-4">
                                        <Title>
                                            {squares.online_workshops.title}
                                        </Title>

                                        <p className="mb-4 text-300">
                                            {
                                                squares.online_workshops
                                                    .description
                                            }
                                        </p>
            ...
            <Col xs={12} className="Home p-0">
                <div className="position-relative min-vh-100">
                    <div className="position-absolute h-100 w-100">
                        ...
                        >
                            <h1 className="display-4 text-700">
                                {banner.body.title}
                            </h1>
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
    get: () => dispatch(getHome() as unknown as UnknownAction),
});
