...
const underCarouselBlock = ({
    children,
    title,
    icon,
    className,
}: React.ComponentProps<"div"> & { icon: string; border?: boolean }) => (
    <Col
        sm={6}
        lg={3}
    ...
    >
        <div>
            <div className="mb-3">
                <i
                    className={
                        "fas fa-" + icon + " fa-3x bg-text bg-dark-gradient"
                    }
                />
            </div>
            <h4
                className="text-700 mb-3 bg-text bg-dark-gradient"
                style={{ textShadow: "0 0 4px 2px black" }}
            >
                {title}
            </h4>
        </div>
