...
class Product extends Component<
    State & { get?: (id: string) => void; match: { params: { slug: string } } }
> {
    componentDidMount() {
    ...
    render() {
        if (!this.props.frontend?.products) return;
        const {
            frontend: {
                products: { loading, message, error, product },
            },
        } = this.props;
        let content;
        ...
        if (loading)
            content = (
                <>
                    <Banner
                        items={[{ to: "/products", content: "Products" }]}
                        title="Loading..."
                    />
                        ...
                        <div className="py-5">
                            <Container>
                                <Row>
                                    ...
                                    <Col md={6}>
                                        <h1 className="bg-dark-gradient bg-text texet-700">
                                            {name[lang]}
                                        </h1>
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
    get: (slug: string) =>
        dispatch(getProduct(slug) as unknown as UnknownAction),
});
