...
export default ({
        ...
        <>
            ...
            <ModalBody className="text-center">
                ...
                <div>
                    ...
                    <Button color="red" onClick={modalToggle}>
                        {close_} <FontAwesomeIcon icon={faTimes} fixedWidth />
                    </Button>
                </div>
            </ModalBody>
        </>
    ...
    const sideDrawerItem = (
        ...
        path: string,
        custom: { link: string; text: string | undefined }[] | false = false,
        addon: [] = []
    ) => {
        if (id && fixed)
            return (
        ...
        if (fixed) {
            let items: { link: string; text: string | undefined }[] | false =
                [];
        ...
        if (id) {
            const feature = role_.features.find(
                (f) => "/user/" + f.prefix === path
            );
        ...
        if (!custom) {
            if (permissions && permissions.includes("c"))
                items.push({ link: "/add", text: dropdown?.add });
            items.push({ link: "/", text: dropdown?.index });
            items = items.concat(addon);
        } else items = custom;
                ...
                <>
                    ...
                    >
                        ...
                        {data.notifications?.filter(
                            ...
                            <Badge
                                color="green"
                                className="position-relative rounded-circle text-x-small text-700 d-inline-flex justify-content-center align-items-center"
                                style={{
                                    width: 18,
                                    height: 18,
                                    top: -2,
                    ...
                    <div className="mt-5 pt-5">
                        <div
                            className="py-2 px-3 text-16 text-300 text-gray rounded-4"
                            onClick={modalToggle}
        ...
        case "admin":
            sideDrawerItems = (
                <>
                    ...
                    <div className="mt-5 pt-5">
                        <div
                            className="py-2 px-3 text-16 text-300 rounded-4"
                            onClick={modalToggle}
                            style={{ cursor: "pointer" }}
                        >
    ...
    return (
        <div
            className={`App SideDrawer nav-left-sidebar bg-soft shadow-sm position-fixed`}
        >
            <div
                className={`vh-100 d-flex flex-column bg-${
                    dark
                        ? "grayblue text-white border-right border-darkblue"
                        : "black text-white"
                }`}
            >
                <div className="px-4 border-bottom border-light bg-black">
                    <div className="mx-3 toolbar-height d-flex align-items-center">
                        ...
                        <div className="ml-auto d-md-none">
                            <i
                                className="fas fa-arrow-left text-21 cursor-pointer"
                                onClick={toggle}
                            />
                        </div>
                    </div>
                    ...
                    <div className="mx-3">
                        <div className="mb-1 mb-md-2 mb-xxl-3 d-flex justify-content-center">
                            <div className="position-relative">
                                ...
                                <div
                                    className="position-absolute border border-2 border-black bg-gray rounded-circle"
                                    style={{
                                        top: 2,
                                        right: 5,
                                        width: 18,
                        ...
                        <div className="text-center text-18 mb-3 mb-md-4 mb-xxl-5 pb-2 pb-md-3 pb-xxl-4">
                            Welcome {roles_[role]}
                            <Col xs={12} className="p-0 text-700">
                                {name}
                            </Col>
                        </div>
            ...
            <div
                className="backdrop bg-soft-50 position-fixed d-md-none"
                onClick={toggle}
